...
def polymap(minLat, minLon, maxLat, maxLon, stations, walks, joinedRoutes=None, showStations=False, showWalks=True,
            showRoutes=True):
    html = """<!DOCTYPE html>
	<html>
	  <head>
    ...
    html = html.replace(":minLat", str(minLat)).replace(":maxLat", str(maxLat)).replace(":minLon", str(minLon)).replace(
        ":maxLon", str(maxLon)).replace(":stationJson", stationJson).replace(":walks", walkJson)
...
def buildGraph(agencies):
    ...
    for override, folder, prepend in agencies:
        ...
        for row in namesReader:
            ...
            if len(row) > 2:
                stop_name = row[2]
            names[row[0]] = {"name":row[1],"short_name":stop_name}
    ...
    for tripId in tripToStops:
        ...
        for position in range(len(stops)):
            ...
            if source != None:
                ...
                # print ktime
                if ktime < 0:
                    print "arrive[" + target["id"] + "]", target['arrive'], "-", "depart[" + source['id'] + "]", source[
                        'depart']
                    for k in stops:
                        print k
                else:
    ...
    for source in paths:
        for target in paths[source]:
            ...
            c.execute("INSERT INTO shortest_route_path(source,target,nodes,hop_count) values(?,?,?,?)",
                      (source, target, nodes, len(paths[source][target])))
        conn.commit()
        ...
        for id in range(len(tripToStops[tripId])):
            ...
            if folder.endswith("metro-north"):
                ...
                if "block_id" in trip and len(trip["block_id"]) != 0:
                    ...
                    blockId = split[-4:]
            c.execute(
                "INSERT INTO nested_trip(lft,rgt,trip_id,service_id,stop_id,depart,arrive,block_id,route_id,fare_type) values(?,?,?,?,?,?,?,?,?,?)",
                (id + 1, len(tripToStops[tripId]), tripId, trip["service_id"], stop["id"], stop["depart"],
                 stop["arrive"], blockId, trip["route_id"], fareType))
            lastStation = stop
        conn.commit()
        for id in ids:
            for ids2 in ids:
    ...
    for row in connectionsReader:
        ...
        c.execute("SELECT level from schedule_path where source=? and target=? order by level desc limit 1",
                  (depart, arrive))
        pos = 0
        row2rows = c.fetchall()
    ...
    for stop in stations:
        ...
        short_name = None
        if stop["id"] in names:
            name = names[stop["id"]]["name"]
            short_name = names[stop["id"]]["short_name"]
        altId = None
        if stop["id"] in alternate_ids:
            altId = alternate_ids[stop["id"]]
        c.execute("INSERT INTO stop(name,short_name,stop_id,lat,lon,departure_vision,alternate_id) values(?,?,?,?,?,?,?)",
                  (name, short_name,stop["id"], stop["lat"], stop["lon"], dv, altId))
    conn.commit()
...
c2 = conn2.cursor()
c2.execute(
    "select source,target,adult,child,senior,disabled,weekly,ten_trip,monthly,student_monthly,adult_offpeak,weekly_offpeak,ten_trip_offpeak,onboard_offpeak,onboard from fares")
        ...
        for station in stations:
            ...
            for stationp in stationsp:
                ...
                if dist < 250:
                    ...
                    c.execute(
                        "INSERT INTO agency_transfer_edge(agency_source,agency_target,source,target,duration) values(?,?,?,?,?)",
                        (agency, agencyp, station["id"], stationp["id"], int(round(dist * 1.389))))
        conn.commit()
conn.close()
